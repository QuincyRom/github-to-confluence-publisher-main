...
def createPage(title, content, parentPageID, login, password):
    ...
    newPagejsonQuery["body"]["storage"]["value"] = (
        "<b>This page is auto generated by the inQdo GitHub Converter. To edit, please make changes in the GitHub repository: </b>"
        + '<b><a href="https://github.com/QuincyRom/github-to-confluence-publisher-main">G-Star</a></b>'
        + content
    )
...
def searchPages(login, password):
    ...
    logging.debug(
        "Calling URL: "
        + str(CONFIG["confluence_url"])
        + "search?cql=parent="
        + str(CONFIG["confluence_parent_page_id"])
        + '+and+text~{"'
...
def deletePages(pagesIDList, login, password):
    ...
    for page in pagesIDList:
        ...
        logging.debug(
            "Calling URL: " + str(CONFIG["confluence_url"]) + "content/" + str(page)
        )
...
def attachFile(pageIdForFileAttaching, attachedFile, login, password):
    ...
    }  # disable token check. Otherwise it will be 443 status code
